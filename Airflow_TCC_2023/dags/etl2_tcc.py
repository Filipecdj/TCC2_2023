...
def trata_colunas(arquivo_num):
    ...
    df = pd.read_csv(f"/opt/airflow/dados/arquivo_saida_{arquivo_num}.csv",
                     header=None,
                     names=colnames,
...
def carga_bigquery(df):
    ...
    df.to_gbq(destination_table='base_tcc.UK_Property', if_exists='append', credentials=credentials)


def executa_bigquery():
    for arquivo_num in range(1, 113):
        df = trata_colunas(arquivo_num)
        carga_bigquery(df)


with DAG('dag_etl_uk', start_date=datetime(2023, 11, 4),
